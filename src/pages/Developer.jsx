...
function ProjectCard({ project, index, dark }) {
  ...
  return (
    <div ref={ref} className={`group border-b py-10 transition-all duration-700 ${dark ? "border-white/8" : "border-black/8"} ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: `${index * 80}ms` }}>
      <div className="flex flex-col md:flex-row md:items-start gap-6 md:gap-12">
        <span className={`text-[11px] tracking-[0.3em] uppercase shrink-0 pt-1 md:w-16 ${dark ? "text-white/25" : "text-black/25"}`}>{project.year}</span>
        <div className="flex-1">
          <div className="flex items-start justify-between gap-4 mb-3">
            <h3 className={`font-display text-2xl transition-colors duration-300 ${dark ? "text-white group-hover:text-[#E8D5B7]" : "text-[#1a1a1a] group-hover:text-[#8B6F47]"}`}>{project.title}</h3>
            <a href={project.link} className={`shrink-0 w-8 h-8 rounded-full border flex items-center justify-center transition-all duration-300 ${dark ? "border-white/15 group-hover:border-white/40 group-hover:bg-white/5" : "border-black/15 group-hover:border-black/40 group-hover:bg-black/5"}`} aria-label="View project">
              <svg className={`w-3 h-3 transition-all duration-300 ${dark ? "text-white/50 group-hover:text-white" : "text-black/40 group-hover:text-black"} group-hover:translate-x-px group-hover:-translate-y-px`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M7 17L17 7M17 7H7M17 7v10" />
              </svg>
            </a>
          </div>
          <p className={`text-[14px] leading-relaxed mb-5 max-w-xl ${dark ? "text-white/45" : "text-black/45"}`}>{project.description}</p>
    ...
    <main className="pt-28 pb-32 px-6 md:px-16 max-w-6xl mx-auto">
      <div ref={headerRef} className="mb-24">
        <div className={`transition-all duration-700 ${headerInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          <span className={`text-[11px] tracking-[0.35em] uppercase block mb-6 ${dark ? "text-white/30" : "text-black/30"}`}>Software Development</span>
          <h1 className={`font-display text-[clamp(3rem,7vw,6rem)] leading-[0.92] mb-8 ${dark ? "text-white" : "text-[#1a1a1a]"}`}>
            Building things<br />
            <span className={dark ? "text-white/30" : "text-black/25"}>that matter.</span>
          </h1>
          <p className={`max-w-lg text-[15px] leading-relaxed ${dark ? "text-white/45" : "text-black/45"}`}>
      ...
      <div ref={skillsRef} className="mb-24 grid md:grid-cols-3 gap-6">
        {skills.map(({ category, items }, i) => (
          <div key={category} className={`p-8 rounded-2xl border transition-all duration-700 ${dark ? "border-white/8 bg-white/[0.02]" : "border-black/8 bg-black/[0.02]"} ${skillsInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`} style={{ transitionDelay: `${i * 100}ms` }}>
            <p className={`text-[11px] tracking-[0.3em] uppercase mb-5 ${dark ? "text-white/30" : "text-black/30"}`}>{category}</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <span className={`text-[11px] tracking-[0.35em] uppercase ${dark ? "text-white/30" : "text-black/30"}`}>Selected Projects</span>
